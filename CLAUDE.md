...
- **Tool layer** (`hooks/hooks.json` + a Python script). Fires on Claude's tool
  ...
  - `Stop`: `decision: "block"` plus `reason` makes Claude continue instead of
    stopping (`paste-guard`). No tool payload at all: the hook reads the turn's
    text from `transcript_path`, and `stop_hook_active` must stand the hook
    down or every block loops forever.
...
`session-logger` are tool-layer only; `paste-guard` is chat-layer only (a `Stop`
hook, no tool events); `commit-style` is git-layer only (no `hooks.json`);
